...
{
    public class StorageService_FileMerge3 : StorageService
    {
        //Don't change this!!!
        private static readonly string rootFolderPathFormat = "SaveGame/FileMerge3/db.{0}";
        ...
        public override SGJsonNode LoadDatabaseDataJson(Database database)
        {
            ...
            {
                return SGJson.Parse(Base64ToContent(dictFileContent[dbFileNameNoExtension]));
            }
        }
        ...
        public override SGJsonNode LoadNodeDataJson(SGNode node)
        {
            lock (database)
            {
                ...
                {
                    return SGJsonNode.Parse(Base64ToContent(dictFileContent[fileName]));
                }
            }
        }
        ...
        string ContentToBase64(string content)
        {
            byte[] bytes = Encoding.UTF8.GetBytes(content);
            return Convert.ToBase64String(bytes);
        }

        string Base64ToContent(string base64)
        {
        ...
        bool UpdateDatabaseToDisk()
        {
            ...
            try
            {
                ...
                dictFileContent[dbFileNameNoExtension] = ContentToBase64(jNode.ToString());
                return true;
            }
            catch (Exception e)
            {
        ...
        bool UpdateNodeToDisk()
        {
            ...
            foreach (var kv in dictJson)
            {
                try
                {
                    ...
                    dictFileContent[fileName] = ContentToBase64(kv.Value.ToString());
                }
                catch (Exception e)
                {
